...
class _ActivityFormScreenState extends State<ActivityFormScreen> {
  ...
  void initState() {
    ...
    _stdListController = TextEditingController(
      text: widget.activity?.stdList.join(", ") ?? '',
    );
  }
  ...
  void _saveActivity() async {
    // kept for backward-compat, but prefer using _confirmAndSave
  ...
  Future<void> _confirmAndSave() async {
    ...
    final action = widget.activity == null ? 'Save' : 'Update';
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: Text('$action Activity'),
        content: Text('Do you want to $action this activity?'),
        actions: [
          TextButton(
    ...
    List<String> stdList = _stdListController.text
        .split(',')
        .map((e) => e.trim())
          ...
          child: Column(
            children: [
              ...
              ElevatedButton(
                ...
                child: Text(widget.activity == null ? 'Save' : 'Update'),
              ),
            ],
          ),
        ),
